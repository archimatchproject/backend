...
from app.announcement.serializers.utils.PieceRenovateSerializer import PieceRenovateSerializer
from app.announcement.serializers.utils.ProjectExtensionSerializer import ProjectExtensionSerializer
from app.announcement.serializers.utils.ProjectImageSerializer import ProjectImageSerializer
from app.announcement.serializers.utils.ArchitectSpecialitySerializer import ArchitectSpecialitySerializer
from app.announcement.serializers.utils.ProjectCategorySerializer import ProjectCategorySerializer
from app.announcement.serializers.utils.PropertyTypeSerializer import PropertyTypeSerializer
from app.announcement.serializers.utils.AnnouncementWorkTypeSerializer import AnnouncementWorkTypeSerializer
...
class AnnouncementInputSerializer(serializers.ModelSerializer):
    ...
    pieces_renovate = serializers.PrimaryKeyRelatedField(queryset=PieceRenovate.objects.all(), many=True)
    project_extensions = serializers.PrimaryKeyRelatedField(queryset=ProjectExtension.objects.all(), many=True)
    project_images = serializers.PrimaryKeyRelatedField(queryset=ProjectImage.objects.all(), many=True, required=False)

    class Meta:
        model = Announcement
        fields = [
            'id', 'client', 'architect_speciality', 'needs', 'project_category',
            'property_type', 'work_type', 'pieces_renovate', 'address', 'city',
            'terrain_surface', 'work_surface', 'budget', 'description',
            'architectural_style', 'project_extensions', 'project_images'
        ]
