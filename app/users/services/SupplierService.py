...
class SupplierService:
    ...
    def supplier_first_connection(self,request):
        try :
            ...
            data = request.data
            request_keys = set(data.keys())
            expected_keys = {'type', 'company_name', 'address', 'phone_number','speciality',"id"}
    ...
    def supplier_update_profile(self,request):
        try :
            ...
            data = request.data
            request_keys = set(data.keys())
            expected_keys = { 'company_name', 'address', 'phone_number','speciality',"id"}
            SupplierService.handle_user_data(request_keys,expected_keys)
    ...
    def supplier_update_links(self,request):
        try :
            ...
            if not supplier.social_links:
                supplier.social_links = supplier.social_links = SocialMedia.objects.create(**data)
                supplier.save()

            else:
                social_links = supplier.social_links
                supplier.social_links = SocialMedia.objects.filter(id=social_links.id).update(**data)
                supplier.save()
